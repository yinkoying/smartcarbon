...
網頁應用程式: index.html (整合 Tailwind CSS, Chart.js)。

後端運算函數: lambda_function.py (Python 3.x)。
...
輸入範疇一、二、三之活動數據 (油、電、水等)，計算總碳排量。

依據 ISO 14064-1 係數進行運算：
...
透過 ?action=export 參數觸發，生成 Excel 可讀格式。
...
AWS DynamoDB: 建立資料表 CarbonRecords，主鍵設為 record_id (字串)。
